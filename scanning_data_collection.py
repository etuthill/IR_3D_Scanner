...
with open(csv_path, mode='w', newline='') as csvfile:  # create csv
    ...
    writer.writerow(["rowID", "pan", "tilt", "irValue"])  # headers
    ...
    while True:
        ...
        if len(parts) == 4:
            # unpack into ints and put into the csv
            rowID, pan, tilt, irValue = (int(p) for p in parts)
            writer.writerow([rowID, pan, tilt, irValue])
            csvfile.flush()
